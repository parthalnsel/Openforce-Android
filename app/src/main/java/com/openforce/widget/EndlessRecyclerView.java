...
public final class EndlessRecyclerView extends RecyclerView {
        ...
        private static LayoutManagerResolver getResolver(@NonNull LayoutManager layoutManager) {
            if (layoutManager instanceof LinearLayoutManager) {
                return new LayoutManagerResolver() {
                    @Override
                    public int findLastVisibleItemPosition(@NonNull LayoutManager layoutManager1) {
                        return ((LinearLayoutManager) layoutManager1).findLastVisibleItemPosition();
                    }
                };
            } else if (layoutManager instanceof StaggeredGridLayoutManager) {
                return new LayoutManagerResolver() {
                    @Override
                    public int findLastVisibleItemPosition(@NonNull LayoutManager layoutManager12) {
    ...
    private final class EndlessScrollListener extends OnScrollListener {
        ...
        public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
            ...
            if (pager.shouldLoad() && lastItemPosition - lastVisibleItemPosition <= threshold && !pager.isLoading()) {
                setRefreshing(true);
                pager.loadNextPage();
            }
        }
